...
module {
  func @graph() {
    ...
    ^bb0(%arg0: i32):  // no predecessors
      ...
      %14 = "equeue.alloc"(%12) {data_bit = 32 : i64, shape = dense<[1, 3, 3, 10]> : tensor<4xi64>} : (i32) -> memref<1x3x3x10xf32>
      "equeue.add_comp_field"(%arg0, %14) {names = "wbuffer "} : (i32, memref<1x3x3x10xf32>) -> ()
      %15 = "equeue.alloc"(%12) {data_bit = 32 : i64, shape = dense<[1, 5, 5, 10]> : tensor<4xi64>} : (i32) -> memref<1x5x5x10xf32>
      "equeue.add_comp_field"(%arg0, %15) {names = "obuffer "} : (i32, memref<1x5x5x10xf32>) -> ()
      %16 = subview %13[0, 0, 0, 0] [1, 3, 3, 3] [1, 1, 1, 1]  : memref<1x7x7x3xf32> to memref<1x3x3x3xf32, #map3>
      affine.for %arg1 = 0 to 10 step 14 {
