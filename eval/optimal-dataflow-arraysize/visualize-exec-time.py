...
for net in network_settings:
  ...
  energy = [
    b
    for a,b,c,d,e,f,g,h in zip(net_cycles, exec_time, read_bw, write_bw, max_sram_read, max_sram_write, max_n_sram_read, max_n_sram_write)]
  cycle = [a
    for a,b,c,d,e,f,g,h in zip(net_cycles, exec_time, read_bw, write_bw, max_sram_read, max_sram_write, max_n_sram_read, max_n_sram_write)]
...
ell_offset = ScaledTranslation(xx, yy, ax.transScale)
...
ax.add_patch(Ellipse(xy=(0, 0), width=0.45, height=1.2, color="r", fill=False, lw=2, zorder=5, transform=ell_tform))
