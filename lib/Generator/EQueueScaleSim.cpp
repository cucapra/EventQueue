...
void MLIRGenImpl::scaleSimGenerator(){
  ...
  if(accel_config.dataflow==DataFlow::IS){
    ...
    int max_cols_per_v_fold = max_parallel_window * accel_config.array_width
    ...
    for(int i = 0; i < num_v_fold; i++){//seq_for  
          ...
          for(int j = 0; j < num_h_fold; j++){//seq_for
            ...
            for(int t = 0; t < row_this_fold-1; t++){//seq_for
              ...
              for(int r = 0; r < row_this_fold-1; r++){//par_for
                ...
                for(int c = 0; c < col_this_fold; c++){//par_for
                  ...
                  auto pe_res = LaunchOpBuilder(start_cpy, procs[r][c], ValueRange{ibuffer2s[r][c]}, 
                    [&](ValueRange ins){
                    input = read_op(ins[0], ValueRange{});
                    return_op(ValueRange{input});
                  });
                  // what are pes[0] and [1] here
                  filter_cpy = pe_res[0]; 
                  ifmap_flight_line.push_back(pe_res[1]);
                  if(c==0 && r==0){
                    prev_filter_cpy=control_and(ValueRange{input_copy});
                  }else{
                    prev_filter_cpy = control_and(ValueRange{prev_input_cpy, input_cpy});
                  }
                }
              ...
              for(int r = 0; r < row_this_fold-1; r++){//par_for
                for(int c = 0; c < col_this_fold; c++){//par_for
                  ...
                  if(c==0 && r==0){
                    prev_filter_cpy=control_and(ValueRange{input_cpy});
                  }else{
                    prev_filter_cpy = control_and(ValueRange{prev_input_cpy, input_cpy});
                  }
                }
            ...
            for(int i = 0; i < e2 + accel_config.array_width + accel_config.array_height; i++){
              num = min(i, accel_config.array_height)
              if (i > e2 + accel_config.array_height){num = 0}
              // copying weights in
              start_weight_copy = start_op()
              for(int n = 0; num ; n++){
              ...
              wait_op(ValueRange{prev_input_cpy});
              //---------------------------------
                  ...
                  if(c!=col_this_fold-1 && r!=row_this_fold-1){
                    ...
                    compute_signal = LaunchOpBuilder(start_compute_signal, procs[r][c], ValueRange{
                      ...
                      if( r+c <= t && // too early, nothing to pass
                          t <= e2 + accel_config.array_heigh + c// too late, nothing to pass
                          t%e_fresh_px == 0
                      ){
