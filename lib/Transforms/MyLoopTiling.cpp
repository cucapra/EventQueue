...
struct MyLoopTiling : public PassWrapper<MyLoopTiling, FunctionPass>  {
  ...
  MyLoopTiling() = default;
  MyLoopTiling(const MyLoopTiling& pass) {}
  // explicit MyLoopTiling(uint64_t cacheSizeBytes, bool avoidMaxMinBounds = true)
  //     : avoidMaxMinBounds(avoidMaxMinBounds) {
  //   this->cacheSizeInKiB = cacheSizeBytes / 1024;
  // }

  
  Option<uint64_t> cacheSizeInKiB{*this, "cache-size", llvm::cl::desc("..."), llvm::cl::init(512)};
  Option<bool> separate{*this, "seperate", llvm::cl::desc("...")};
  Option<unsigned> tileSize{*this, "tile-size", llvm::cl::desc("...")};
  ListOption<unsigned> tileSizes{*this, "tile-sizes", llvm::cl::desc("..."), llvm::cl::ZeroOrMore, llvm::cl::MiscFlags::CommaSeparated};
...
//   return std::make_unique<MyLoopTiling>(cacheSizeBytes);
// }
// std::unique_ptr<OperationPass<FuncOp>> mlir::createLoopTilingPass() {
//   return std::make_unique<MyLoopTiling>();
// }
...
LogicalResult
MytilePerfectlyNested(MutableArrayRef<AffineForOp> input,
                          ArrayRef<unsigned> tileSizes,
...
void MyLoopTiling::getTileSizes(ArrayRef<AffineForOp> band,
                              SmallVectorImpl<unsigned> *tileSizes) {
  if (band.empty())
    return;
...
void MyLoopTiling::runOnFunction() {
...
constexpr unsigned MyLoopTiling::kDefaultTileSize;

void equeue::registerTilingPass() {
    PassRegistration<MyLoopTiling>(
      "loop-tile",
      "...");
}
